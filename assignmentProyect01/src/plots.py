...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    plt.title("Relationship between Order Weight and Freight Value")
    plt.tight_layout()
    plt.show()
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    plt.plot(df_sorted["date"], df_sorted["order_count"], label="Order Count")
    
    holidays = df_sorted[df_sorted["holiday"] == True]["date"]
    for holiday_date in holidays:
        plt.axvline(holiday_date, color='red', linestyle='--', alpha=0.5)
    
    plt.xlabel("Date")
    plt.ylabel("Number of Orders")
    plt.title("Orders per day with holidays indicated by red vertical lines")
